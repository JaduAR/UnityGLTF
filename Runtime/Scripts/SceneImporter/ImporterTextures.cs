	...
	public partial class GLTFSceneImporter
	{
		...
		private async Task CreateNotReferencedTexture(int index)
		{
			    ...
			    && string.IsNullOrEmpty(Root.Textures[index].Source.Value.Uri))
			{
				...
				await ConstructTexture(Root.Textures[index], index, !KeepCPUCopyOfTexture, true, CompressTextures, false);
			}
		}
		...
		private async Task<TextureData> FromTextureInfo(TextureInfo textureInfo, bool isNormal)
		{
			...
			await ConstructTexture(textureId.Value, textureId.Id, !KeepCPUCopyOfTexture, true, CompressTextures, isNormal);
		...
		protected async Task ConstructImage(GLTFImage image, int imageCacheIndex, bool markGpuOnly, bool compress, bool isLinear, bool isNormal)
		{
			...
			if (_assetCache.ImageCache[imageCacheIndex] == null)
			{
				...
				await ConstructUnityTexture(stream, markGpuOnly, isLinear, compress, isNormal, image, imageCacheIndex);
			}
		}
		...
		async Task<Texture2D> CheckMimeTypeAndLoadImage(GLTFImage image, Texture2D texture, NativeArray<byte> data, bool markGpuOnly, bool compress, bool isLinear)
		{
			switch (image.MimeType)
			{
				...
				case "image/jpeg":
					...
					texture.LoadImage(data.ToArray(), markNonReadable: false);
					break;
				case "image/exr":
		...
		protected virtual async Task ConstructUnityTexture(Stream stream, bool markGpuOnly, bool isLinear, bool compress, bool isNormal, GLTFImage image, int imageCacheIndex)
		{
			...
			if (_nativeBuffers.TryGetValue(stream, out var nativeData))
			{
				...
				await YieldOnTimeoutAndThrowOnLowMemory();
				texture = await CheckMimeTypeAndLoadImage(image, texture, bufferView.bufferData, markGpuOnly, CompressTextures, isLinear);
			...
			else if (stream is MemoryStream)
			{
				using (MemoryStream memoryStream = stream as MemoryStream)
				{
					...
					{
						texture = await CheckMimeTypeAndLoadImage(image, texture, memoryStreamData, markGpuOnly, isLinear, compress);
					}
				}
			}
			else
			{
				...
				{
					texture = await CheckMimeTypeAndLoadImage(image, texture, bufferNative, markGpuOnly, isLinear, compress);
				}
			}
		...
		public virtual async Task LoadTextureAsync(GLTFTexture texture, int textureIndex, bool markGpuOnly, bool isLinear, bool compress)
		{
			try
			{
				...
				await ConstructTexture(texture, textureIndex, markGpuOnly, isLinear, compress, false);
			}
			finally
			{
				lock (this)
		...
		protected virtual async Task ConstructTexture(GLTFTexture texture, int textureIndex, bool markGpuOnly, bool isLinear, bool compress, bool isNormal)
		{
			if (_assetCache.TextureCache[textureIndex].Texture == null)
			{
				...
				await ConstructImage(image, sourceId, markGpuOnly, isLinear, compress, isNormal);
